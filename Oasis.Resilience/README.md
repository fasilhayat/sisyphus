...
> Last measured: May 2026 · 126 tests · net8.0 / net9.0
...
The remaining ~11% of lines are DI wiring edge cases and actor crash-recovery paths that require a live Akka.NET system to exercise. Well above the industry average for a framework of this complexity.

### 🔀 Cyclomatic Complexity

| Metric | Result |
|--------|--------|
| Public methods analysed | 20 |
| Methods exceeding CC = 10 | **0** |
| Highest CC in any method | **5** |
| Average CC | ~2.5 |
| **Score** | **10 / 10** |

Every method stays well within the limit of 10. Logic is decomposed into small, single-purpose helpers throughout the actor and proxy layers.

### 📦 Package Design & Correctness

| Area | Finding |
|------|---------|
| AOP interception | `DispatchProxy` — zero runtime code-gen, works with any .NET DI container |
| Actor model usage | Correct: `Sender` captured before every `await`; no shared mutable state |
| Circuit breaker | HalfOpen → Open immediate re-open; per-operation-key state |
| Retry backoff | Exponential + jitter + `MaxDelayMs` clamp + overflow guard |
| CB + Retry composition | Fully functional end-to-end |
| Cancellation | `CancellationToken` flows through every proxy path |
| Exception transparency | `TargetInvocationException` unwrapped so callers see the real exception type |
| Dead code | None |
| **Score** | **8.7 / 10** |
...
| Area | Finding |
|------|---------|
| Declarative attributes | `[Retry]`, `[CircuitBreaker]`, `[Supervision]`, `[FanOut]` — fully composable |
| Pipeline order | FanOut → CircuitBreaker → Retry → Supervision → method (documented) |
| Log level | DI-configurable via `AddResilience(options => options.LogLevel = ...)` |
| Ask timeout | DI-configurable via `RetryOptions.AskTimeout` |
| Consumer boilerplate | One attribute per method. Zero infrastructure code on the caller side. |
| **Score** | **8.4 / 10** |

### ⏱️ Execution Overhead

| Concern | Detail |
|---------|--------|
| Happy path | One `DispatchProxy.Invoke` + one Akka.NET `Ask` round-trip — ~0.1–0.5 ms overhead in-process |
| Actor system startup | One-time cost at DI build; supervisor actors are cached per method |
| Fan-out workers | Spawned on demand per call — acceptable for I/O-bound parallel work |
| Retry scheduling | `ITimerScheduler` (no thread blocking between attempts) |
| Circuit breaker state | `ConcurrentDictionary` lookup — nanosecond cost on the hot path |
| **Score** | **8.0 / 10** |

The Akka.NET `Ask` per call is the unavoidable cost; it is justified by the isolation, fault-tolerance, and supervision guarantees the actor model provides.

### 📖 Developer Experience

| Area | Finding |
|------|---------|
| README | Mermaid diagrams per pattern, plain-English when/why, code examples |
| Configuration | Single `AddResilience()` call; all knobs optional with sensible defaults |
| Demo project | Covers all four patterns with realistic scenarios |
| XML doc comments | All public types and members documented |
| Error messages | `CircuitBreakerOpenException` includes operation key and remaining time |
| **Score** | **9.0 / 10** |
...
| Dimension | Score |
|-----------|-------|
| Test coverage & stability | 8.5 / 10 |
| Cyclomatic complexity | 10 / 10 |
| Package design & correctness | 8.7 / 10 |
| AOP utilisation | 8.4 / 10 |
| Execution overhead | 8.0 / 10 |
